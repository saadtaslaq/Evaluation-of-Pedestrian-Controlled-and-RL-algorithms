...
# # # from envs.utils.pedestrian import Pedestrian
# # # from envs.utils.info import Deadlock
...
# #         # self.collision_detected = None #added for robot
...
# #         self.ped_collision_penalty = config.getfloat('reward', 'ped_collision_penalty')
...
# #             action, update=False)
# #         return next_human_statefs, reward, done, info
...
# # # /home/caris/pepper3venv/lib/python3.6/site-packages/crowd_sim/envs/crowd_sim.py
        
# #         ################################### START OF CODE FOR PEDESTRIAN ADDED ###########################
# #         # collision with obstacle
...
# #         ped_start_idx_x = ped_idx_map_x - ped_size_map
# #         ped_end_idx_x = ped_start_idx_x + ped_size_map * 2
# #         ped_start_idx_x = max(ped_start_idx_x, 0)
# #         ped_end_idx_x = min(
# #             ped_end_idx_x, int(
# #                 round(
# #                     self.map_size_m / self.map_resolution)))
# #         ped_start_idx_y = ped_idx_map_y - ped_size_map
# #         ped_end_idx_y = ped_start_idx_y + ped_size_map * 2
# #         ped_start_idx_y = max(ped_start_idx_y, 0)
# #         ped_end_idx_y = min(
# #             ped_end_idx_y, int(
# #                 round(
# #                     self.map_size_m / self.map_resolution)))
# #         if end_idx_x > ped_start_idx_x and end_idx_y > ped_start_idx_y:
# #             map_around_ped = self.map[ped_start_idx_x:ped_end_idx_x,
# #                                         ped_start_idx_y:ped_end_idx_y]
# #             if np.sum(map_around_ped) < map_around_ped.size:
# #                 collision = True

# #         # collision with border
# #         if border is not None:
# #             if px <= border[0][0] + human.radius or \
# #                px >= border[0][1] - human.radius or \
# #                py <= border[1][0] + human.radius or \
# #                py >= border[1][1] - human.radius:
# #                 ped_collision = True
        
# #         # Deadlock, see if ped is closer to any obstacle than the discomfort dist
...
# #                     self.map_size_m / self.map_resolution)))
# #         ped_start_idx_y = ped_idx_map_y - ped_size_map
# #         end_idx_y = start_idx_y + ped_size_map * 2
# #         start_idx_y = max(start_idx_y, 0)
# #         end_idx_y = min(
# #             end_idx_y, int(
# #                 round(
...
# #         ################################### END OF CODE FOR PEDESTRIAN ADDED ###########################

# #         # collision detection between humans
# #         ped_collision_each_other = False
...
# #         # success_ped = 0
# #         # check if all pedestrians reach their goal
# #         # for i, human in enumerate(self.humans):
# #         #     print(human)
# #         #     ped_end_position = np.array(
# #         #         human.compute_position(
# #         #             action, self.time_step))
# #         #     ped_reaching_goal = norm(
# #         #         ped_end_position -
# #         #         np.array(
# #         #             human.get_goal_position())) < human.radius
# #         #     print("ped_reaching_goal = ",ped_reaching_goal)
# #         #     if ped_reaching_goal == True:
# #         #         print("YESSS MATE")
...
# #         elif reaching_goal == True:
# #             print("Robot reached goal    YESSS MATE")
# #             reward = self.success_reward
# #             # success += 1
# #             done = True
# #             info = ReachGoal()
...
# #             reward = self.ped_collision_penalty
...
#         # if self.config.robot.appearance == False:
...
#         elif self.human_policy == 'SF':
#             policy_config_file = os.path.join(
#                 self.human_policy, 'policy.config')
#             policy_config = configparser.RawConfigParser()
#             policy_config.read(policy_config_file)
#             self.policy.configure(policy_config)
#             self.policy.set_env(self)
...
#         if self.robot.policy.name != 'SDOADRL' and self.robot.policy.name != 'ORCA' and self.robot.policy.name != 'SF':
#             self.create_observation_from_static_obstacles(obstacles)
...
#             larger_map_around_robot = self.map[start_idx_x:end_idx_x,
#                                                start_idx_y:end_idx_y]
#             if np.sum(larger_map_around_robot) < larger_map_around_robot.size and (math.floor(self.robot.v_pref) == 0):
#                 closeToObstacle = True
...
#             if (np.sum(larger_map_around_robot) < larger_map_around_robot.size) and (math.floor(self.robot.v_pref) == 0):
#                 deadlock = True
#                 self.map_list.append(1)
#                 true += 1
#                 self.deadlock_state.append(deadlock)
#                 print(" self.deadlock_state = ===== ",  self.deadlock_state)


#                 print("", np.mean(true))
            

#                 #distance to goal when deadlock
#                 end_position = np.array(
#                     self.robot.compute_position(
#                         action, self.time_step))
#                 distance_to_goal_robot = norm(
#                     end_position -
#                     np.array(
#                         self.robot.get_goal_position()))
#                 print("distance_to_goal = ", distance_to_goal_robot)
#                 self.distance_to_goal.append(distance_to_goal_robot)
#                 print(""" Average numeert is with a stanadrd deviation of {:.2f}+-{:.2f}=== """.format(np.mean(self.distance_to_goal), np.std(self.distance_to_goal)))
#             else:
#                 self.map_list.append(0)
#             #self.map_list=np.array(self.map_list)    
           
#         # collision detection between humans
#         human_collision_detected = 0 #added
#         human_num = len(self.humans)
#         for i in range(human_num):
#             for j in range(i + 1, human_num):
#                 dx = self.humans[i].px - self.humans[j].px
...
#                     # detect collision but don't take humans' collision into
#                     # account
#                     human_collision_detected += 1
#                     print('Number of times humans have collided: ', human_collision_detected)
#                     logging.debug('Collision happens between humans in step()')

        
#         print("""Length of self.deadlock_state =  {:.2f}  Average number of  is with a stanadrd deviation of {:.2f}+-{:.2f}""" .format(len(self.deadlock_state),np.mean(self.deadlock_state),np.std(self.deadlock_state)))
#         # check if reaching the goal
#         timeout = 0
#         reach_goal = 0
#         collisions = 0
...
#             info = Timeout()
#             timeout += 1
#             print('Number of timeouts: ', timeout)
#         elif reaching_goal:
#             reward = self.success_reward
#             done = True
#             info = ReachGoal()
#             #reach_goal += 1
#             print('Number of reached goals: ', reach_goal)
#         elif collision:
#             reward = self.collision_penalty
#             done = True
#             info = Collision()
#             collisions += 1
#             print('Number of collisions (with obstacle): ', collisions)
#         elif collision_other_agent:
#             reward = 0
#             done = True
#             info = CollisionOtherAgent()
#             collision_other += 1
#             print('Number of collisions (with obstacle): ', collision_other)
#         elif closeToObstacle:
#             print("YES DEADLOCK")
#             reward = - self.discomfort_penalty_factor * self.time_step * 0.1
#             done = False
...
#         if update:
#             print("self.map_list", len(self.map_list))
#             print("Find 1s ===", np.where(self.map_list)[0])
#             if hasattr(self.robot, 'attention_weights'):
#                 self.attention_weights.append(self.robot.attention_weights)
...
class CrowdSim(gym.Env):
    ...
    def __init__(self):
        ...
        self.other_robots = None
        self.global_time = None
        self.haman_time = None
        self.human_policy = None

        # reward function
        self.success_reward = None
        self.collision_penalty = None
        self.deadlock_penalty = None
        self.discomfort_dist = None
        self.discomfort_penalty_factor = None
        self.rotation_penalty_factor = None
        self.deadlock = None
    ...
    def configure(self, config, silent=False):
        ...
        self.success_reward = config.getfloat('reward', 'success_reward')
        self.collision_penalty = config.getfloat('reward', 'collision_penalty')
        self.deadlock_penalty = config.getfloat('reward', 'deadlock_penalty')
        ...
        # elif self.human_policy == 'orca' or self.human_policy == 'social_force':
        #     policy_config_file = os.path.join(
        #         self.human_policy, 'policy.config')
        #     policy_config = configparser.RawConfigParser()
        #     policy_config.read(policy_config_file)
    ...
    def generate_random_human_position(
        ...
        if rule == 'square_crossing':
            ...
            self.humans = []
            self.other_robots = []
            # print("human_num CIRCLE CROSSING = ", self.human_num)
            for i in range(self.human_num):
                self.humans.append(
                    self.generate_circle_crossing(
                        'human', policy))
                # print("self.humans CIRCLE CROSSING =====", self.humans)
            print("se")
            for i in range(other_robots_num):
                self.other_robots.append(
                    self.generate_circle_crossing('robot'))
    ...
    def generate_circle_crossing(self, agent_type, policy=None):
        ...
        if self.randomize_attributes:
            agent.sample_random_attributes()
            # print("agent.sample_random_attributes = ", agent.sample_random_attributes())
        while True:
            angle = np.random.random() * np.pi * 2
            # add some noise to simulate all the possible cases robot could
            # meet with human
    ...
    def reset(self, phase='test', test_case=None, imitation_learning=False, compute_local_map=True):
        ...
        if self.case_counter[phase] >= 0:
            # print("yes case_counter[phase] = ",self.case_counter[phase])
            if phase == 'test':
                # print("YES MATE testing mode")
                for i in range(self.human_num):
                    self.human_times = [0] * self.human_num
                    # print("human_number = ", self.human_num)
                    # self.humans.sample_random_attributes()
    ...
    def step(self, action, update=True, compute_local_map=True, border=None):
        ...
        for human in self.humans:
            ...
            if self.human_policy == 'orca':
                ...
                human_actions.append(human.act())
            elif self.human_policy == 'social_forces':
                human_actions.append(human.act(ob, self.obstacle_vertices))               
            else:
                human_actions.append(human.act(ob))
        ...
        for i, human in enumerate(self.humans):
            ...
            if end_idx_x > start_idx_x and end_idx_y > start_idx_y:
                ...
                if (np.sum(map_around_human) < map_around_human.size) and (math.floor(human.v_pref) == 0):
                    ...
                    distance_to_goal_total.append(distance_to_goal_ped)
        ...
        if update:
            ...
            for i, human_action in enumerate(human_actions):
                ...
                # print("i for human_action=", i)
                self.humans[i].step(human_action)
                # print("self.humans before= ", self.humans)
            # for i, robot_action in enumerate(robot_actions):
            #     self.other_robots[i].step(robot_action)
            self.global_time += self.time_step

            for i, human in enumerate(self.humans):
                # print("self.humans after ", self.humans)
                # print("human = ", human)
                # print("i=", i)
        ...
        if compute_local_map:
            if self.use_grid_map:
                ...
                local_map = self.get_local_map_angular(self.robot.get_full_state())
        return ob, local_map, reward, done, info, self.distance_to_goal
