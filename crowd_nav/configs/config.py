...
class Config(object):
    ...
    env = BaseConfig()
    env.env_name = 'NavRepTrainEnv-v0'  # name of the environment
    env.time_limit = 35 # time limit of each episode (second)
    env.time_step = 0.2 # length of each timestep/control frequency (second)
    env.val_size = 100
    ...
    # reward function
    reward = BaseConfig()
    reward.success_reward = 1 #old value was 10
    reward.collision_penalty = -0.25 #old value was -20
    # discomfort distance for the front half of the robot
    reward.discomfort_dist_front = 0.2 #old value was 0.25
    # discomfort distance for the back half of the robot
    reward.discomfort_dist_back = 0.2 #old value was 0.25
    reward.discomfort_penalty_factor = 0.5 #old value was 10
    reward.gamma = 0.99  # discount factor for rewards #old value was 0.99
    #reward.rotation_penalty_factor = -0.003
    ...
    sim.human_num = 7 # total number of humans
    ...
    humans.policy = "orca"
    humans.radius = 1 # radius of each human
    ...
    training.num_env_steps = 100e6  # number of environment steps to train: 10e6 for holonomic, 20e6 for unicycle
    ...
    training.output_dir = 'data/100million'  # the saving directory for train.py
    training.resume = False  # resume training from an existing checkpoint or not
    training.load_path = 'data/eh12/checkpoints/55554.pt'  # if resume = True, load from the following checkpoint
