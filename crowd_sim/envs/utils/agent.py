...
from crowd_sim.envs.utils.action import ActionRot, ActionXYRot, ActionXY
from crowd_sim.envs.utils.state import ObservableState, FullState, FullState_sf
...
class Agent(object):
    ...
    def sample_random_attributes(self):
        ...
        """
        # self.v_pref = np.random.uniform(0.8, 10)
        self.v_pref = 0.8
        # self.radius = np.random.uniform(0.3, 1)
        # self.radius = 0.3
        self.radius = 1 
    ...
    def check_validity(self, action):
        try:
            # print("Aactionnnnn", action)
            # print("\n")
            # print("ActionROT",ActionRot)
            # print("\n") 
            # print("ActionXYROT",ActionXYRot)
            # print("ACTION", action)
            # print("ActionRot---", ActionRot)
            # a = isinstance(action, ActionRot)
            # print("YESSSS SAME INSTANCE", a)
            assert isinstance(action, ActionRot) or isinstance(action, ActionXYRot) or isinstance(action, ActionXY)
    ...
    def step(self, action):
        ...
        if isinstance(action, ActionXY):
            ...
            self.theta = (self.theta + action.r) % (2 * np.pi)
            self.vx = action.vx * np.cos(self.theta)
            self.vy = action.vy * np.sin(self.theta)
        else:
            self.theta = (self.theta + action.r) % (2 * np.pi)
