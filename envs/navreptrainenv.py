...
class NavRepTrainEnv(gym.Env):
    ...
    def __init__(self, scenario='test', silent=False, legacy_mode=False, adaptive=True, lidar_legs=True,
        ...
        self.observation_space = spaces.Tuple((
            ...
            spaces.Box(low=-np.inf, high=np.inf, shape=(10,), dtype=np.float32)
        ))
        # parameters
    ...
    def _add_border_obstacle(self):
        ...
        # for i in self.soadrl_sim.obstacle_vertices:
        ...
        # border_sf = np.array([x_min, x_max, y_min, y_max], dtype=np.int64)
        # self.border_sf = tuple(border_sf)

        self.soadrl_sim.obstacle_vertices.append(border_vertices)


        self.border_sf = [[x_min, x_max, y_min, y_max]]
        
        return self.border_sf
    ...
    def step(self, action):
        ...
        if not self.LEGACY_MODE:
            ...
            # step
            _, _, reward, done, info, distance_to_goal = self.soadrl_sim.step(
        ...
        return obs, reward, done, {'event': info}, distance_to_goal
    
    # SAAD CHANGES    
    def human_step(self, agent = None, n = None):
        num = self.soadrl_sim.human_num
        print("num")
        observation_array = np.zeros([0,6], dtype=int)

        for n, agent in enumerate(self.soadrl_sim.humans): 
            px = agent.px
            py = agent.py
            vx = agent.vx
            vy = agent.vy
            ...
            print("vx----", vx)
            print("vy----", vy)
            observation_array = np.append(observation_array, [[px,py,vx,vy,gx,gy]], axis = 0)
            print("after zero obser", observation_array)

        return observation_array

    def step_sf(self, action = None):
            
        ## FOR ADDING SOCIAL FORCES ##

        observation_array = self.human_step()
        obstacle_array = self._add_border_obstacle()
        ...
        # self.max_size = config('map', 'map_size_m')
        # phase = "test"

        # obstacle_array = CrowdSim.generate_static_map_input(self, max_size = self.max_size, phase = phase, config=config)

        print("NEW obstacle_array--", obstacle_array)

        print("NEW observation_array--", observation_array)
...
#                 self.transform = rendering.Transform()
            ...
            with self.image_lock:
                ...
                self.viewer.draw_circle(r=10, color=(0.5,0.5,0.5))
                ...
                agentcolor = np.array([0.5,0.5,0.5])
                # Green background
...
#                     self.transform.set_scale(20, 20)
                ...
                #         gl.glEnd()
                # Agent body
                for n, agent in enumerate(self.soadrl_sim.humans): #[self.soadrl_sim.robot] + self.soadrl_sim.humans
                    # observation_array = self.human_step(agent == agent, n == n)
                    # print("observation_array render", observation_array)
                    # print("n render", n)
                    px = agent.px
                    py = agent.py
                    # print("PX", px)
                    ...
                    #     # Linear - orange
                    #     # RP - gold
                    #     print('agent policy /n/n/n/n/n/n/n/n',agent.policy)
                    #     if agent.policy=='linear': 
                    #         color=np.array([1, 0.5, 0])
                    #     elif agent.policy=='orca': 
                    #         color=np.array([0.5, 0, 0.5])
                    #     else: 
                    #         color=np.array([0, 0.5, 1])
