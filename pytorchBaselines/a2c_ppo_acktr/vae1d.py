...
class Conv1DVAE(object):
    ...
    def _build_graph(self):
        ...
        with self.g.as_default():
            ...
            self.x = tf.placeholder(tf.float32, shape=[None, N_RAYS, 1])
            # TODO: 3 channels with different features?

            # Encoder
            h = tf.layers.conv1d(
                self.x, 32, 8, strides=4, activation=tf.nn.relu, name="enc_conv1"
            )
            h = tf.layers.conv1d(
            ...
            h = tf.reshape(h, [-1, 4, 1, 256])
            # we use 2d because 1d not available with same API in tf 1.8
            ...
            h = tf.layers.conv2d_transpose(
                h, 64, (6, 1), strides=(4, 1), activation=tf.nn.relu, name="dec_deconv2"
            )
            h = tf.layers.conv2d_transpose(
                h, 32, (9, 1), strides=(4, 1), activation=tf.nn.relu, name="dec_deconv3"
            )
            ...
            self.y = tf.reshape(
                self.y, [-1, N_RAYS, 1]
            )  # due to using 2d we get [batch, N_RAYS, 1, 1]
            ...
            if self.is_training:
                ...
                self.r_loss = tf.reduce_sum(
                    tf.square(self.x - self.y), reduction_indices=[1, 2]
                )
